...
import sys
from pyrogram.errors import UsernameNotOccupied, FloodWait, UserIsBlocked, PeerIdInvalid, UserPrivacyRestricted, UserNotMutualContact, UserDeactivated, UserDeactivatedBan, AuthKeyInvalid, SessionRevoked, AuthKeyUnregistered, UserChannelsTooMuch, PeerFlood
...
def helloworld(c: Client, m: Message):
    c.send_message(m.chat.id, user_guide_text)
...
async def chat_add_people(bot_info, add_users : list, client_chat : str, client_chat_participants : list, owner_id : int):
        ...
        try:
            ...
            if client_chat not in bot_chats:
                ...
                await add_bot.join_chat(chat_id=client_chat)
            ...
            while add_users:
                ...
                if user_for_add not in client_chat_participants:
                    try:
                        ...
                        logger.error(f"{user_for_add} failed attempt, username not uccupied ({bot_name})")
                    finally:
                        await asyncio.sleep(randint(3,9))
...
@bot.on_message(filters.command('get') & filters.user(admins))
...
@bot.on_message(filters.document & filters.command('add_session') & filters.user(admins))
    ...
    if message.document.file_name.endswith('.session'):
        ...
        try:
            ...
            proxy = {
                ...
                "port": 80,
                "username": os.getenv("PROXY_USERNAME_BASE") + "-" + str(proxy_num),
...
@bot.on_message(filters.command(["name_find"]) & filters.user(admins))
...
@bot.on_message(filters.command(["id_find"]) & filters.user(admins))
async def send_info(client, message):
...
@bot.on_message(filters.command(["bots"]) & filters.user(admins))
...
@bot.on_message(filters.command(["bots_info"]) & filters.user(admins))
...
@bot.on_message(filters.command(['guide']) & filters.user(admins))
